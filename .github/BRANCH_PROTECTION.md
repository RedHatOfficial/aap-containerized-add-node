...
- Allow **GitHub Actions** to create matching tags so `release_collection.yml` can publish `vX.Y.Z` with `GITHUB_TOKEN`. Optional `GH_WORKFLOW_KEY` is for `release/*` branch push and the devel PR only. The former tag-triggered `release.yml` is archived under `.github/archived/`.
